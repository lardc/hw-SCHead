...
void SCPC_SaveData(uint16_t Nid, uint16_t Address, uint16_t Data)
{
  ...
  {
    if(SCPC_Data[i].Nid==Nid){ Nid_Count=i; }
  }

  if(Nid_Count>DataTable[REG_TOTAL_SCPC])return; //Данного Nid нет в списке
  //

  //Определения соответсвующего поля для сохранения данных
  switch(Address)
  {
  case REG_SCPC_SC_PULSE_VALUE:         {SCPC_Data[Nid_Count].SC_PulseValue=Data;break;}
  case REG_SCPC_BAT_VOLTAGE:            {SCPC_Data[Nid_Count].BatVoltage=Data;break;}
  case REG_SCPC_DEV_STATE:              {SCPC_Data[Nid_Count].DevState=Data;break;}
  case REG_SCPC_FAULT_REASON:           {SCPC_Data[Nid_Count].DevFault=Data;break;}
  case REG_SCPC_DISABLE_REASON:         {SCPC_Data[Nid_Count].DevDisable=Data;break;}
  case REG_SCPC_WARNING:                {SCPC_Data[Nid_Count].DevWarning=Data;break;}
  case REG_SCPC_PROBLEM:                {SCPC_Data[Nid_Count].DevProblem=Data;break;}
  }
  //
}
//------------------------------------------------------------------------------
...
void SCPC_CapChargeStart(pBCCIM_Interface Interface)
{
  ...
  {
    SCPC_Read_Data(Interface, SCPC_Data[Nid_Count].Nid, true);
    if(SCPC_CheckStatus(Nid_Count, SCPC_Ready)){Nid_Count++;}
...
bool SCPC_GetInfo()
{
  ...
  for(int i=0;i<DataTable[REG_TOTAL_SCPC];i++)
  {
    if(SCPC_Data[i].Nid==DataTable[REG_NID_SCPC_INFO]){ Nid_Count=i; }
    IWDG_Control();
  }

  if(Nid_Count>=DataTable[REG_TOTAL_SCPC])return 0; //Данного Nid нет в списке
  //
